...
@router.get(
    "/",
    tags=["Submenu"],
    description=SubmenuDocs.GET_LIST,
    summary=SubmenuDocs.GET_LIST,
...
@router.get(
    "/{submenu_id}",
    tags=["Submenu"],
    description=SubmenuDocs.GET_DETAIL,
    summary=SubmenuDocs.GET_DETAIL,
...
@router.post(
    "/",
    tags=["Submenu"],
    description=SubmenuDocs.POST_CREATE,
    summary=SubmenuDocs.POST_CREATE,
    response_model=SubmenuOut,
    status_code=201,
)
async def create_submenu(
    menu_id: int, submenu: MenuIn, services: Services = Depends(service_stub)
):
...
@router.patch(
    "/{submenu_id}",
    tags=["Submenu"],
    description=SubmenuDocs.PATCH_UPDATE,
    summary=SubmenuDocs.PATCH_UPDATE,
    response_model=SubmenuOut,
)
async def update_submenu_information(
    ...
    submenu = await services.submenu_service.update(
...
@router.delete(
    "/{submenu_id}",
    tags=["Submenu"],
    description=SubmenuDocs.DELETE,
